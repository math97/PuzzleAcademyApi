...
describe('Load Player Matches (Simple Flow)', () => {
  ...
  it('should fetch matches from both ranked queues', async () => {
    ...
    expect(getMatchesSpy).toHaveBeenCalledWith(
      'puuid1',
      undefined,
      undefined,
      RANKED_SOLO_QUEUE,
  ...
  it('should deduplicate matches from both queues', async () => {
    ...
    riotApiGateway.getMatchesByPuuid = vi
      .fn()
  ...
  it('should create new match and participant for current player', async () => {
    ...
    // Verify
    const savedMatch =
      await inMemoryMatchesRepository.findByRiotMatchId('match1');
  ...
  it('should add participant to existing match if missing', async () => {
    ...
    const player1 = Player.create(
      { name: 'P1', tag: 'T1', riotPuiid: 'puuid1' },
      new UniqueEntityId('player1'),
    );
    const player2 = Player.create(
      { name: 'P2', tag: 'T2', riotPuiid: 'puuid2' },
    ...
    expect(
      savedMatch?.participants.find((p) => p.puuid === 'puuid2'),
    ).toBeTruthy();
