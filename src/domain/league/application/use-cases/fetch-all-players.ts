...
export class FetchAllPlayersUseCase {
    ...
    async execute({ page, limit, from, to }: FetchAllPlayersRequest): Promise<PaginatedResult<PlayerDetailsResponse>> {
        ...
        const data: PlayerDetailsResponse[] = await Promise.all(
            result.data.map(async (player) => {
                const snapshots = await this.snapshotRepository.findByPlayerIdAndDateRange(player.id.toString(), startDate, endDate);
                const firstEverSnapshot = await this.snapshotRepository.findFirstByPlayerId(player.id.toString());
